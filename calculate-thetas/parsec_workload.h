...
/* this enum is for the experiments running on the system */
enum task_name {
	DEDUP = 0,
	CANNEAL,
	FFT,
...
char *profile_path = "./output-phases";
...
parsec_get_theta_parser(int cache_itr, int membw_itr, char *task_name)
{
	...
	strcat(full_path_theta, task_name); //error is here, name not fft
	strcat(full_path_theta, "/");
	strcat(full_path_theta, res_str);
	strcat(full_path_theta, "/");
	strcat(full_path_theta, "theta.txt");
	printf("Theta file path: %s\n", full_path_theta);

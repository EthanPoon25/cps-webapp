	    ...
	    int rem_cache, int rem_membw, phase_entry_t *cur_phase)
{
	...
	/* We are obviously going to have a loss of precision, but we can't use floats in hypervisor */
...
phase_entry_t *radiosity_all_res_phase_entries[MAX_CACHE_ITR][MAX_MEMBW_ITR];
int radiosity_all_res_phase_entries_len[MAX_CACHE_ITR][MAX_MEMBW_ITR];
...
free_data() {
	for (int i = 0 ; i < MAX_CACHE_ITR ; i++) {
		for (int j = 0 ; j < MAX_MEMBW_ITR ; j++) {
			...
			free(radiosity_all_res_phase_entries[i][j]);
		}
	}
}

DLL_EXPORT phase_entry_t *get_phase_entries(size_t cache, size_t membw, enum task_name name) //Error here, use this name?
{
	...
	switch(name) {
		...
		case FFT: { //passing in fft name here
		...
		case RADIOSITY: {
			parser = parsec_get_phase_parser(cache, membw, "radiosity");
			all_res_phase_entries     = radiosity_all_res_phase_entries;
			all_res_phase_entries_len = radiosity_all_res_phase_entries_len;
			break;
		}
		default:
	...
	// sanity check
	//entry = all_res_phase_entries[cache][membw];
	//while (entry) {
	//	printf("entry %ld %ld: %p\n", cache, membw, entry);
...
DLL_EXPORT phase_entry_t *get_theta_entries(size_t cache, size_t membw, enum task_name name)
{
	...
	switch(name) {
		...
		case RADIOSITY: {
			parser = parsec_get_theta_parser(cache, membw, "radiosity");
			all_res_phase_entries     = radiosity_all_res_phase_entries;
			all_res_phase_entries_len = radiosity_all_res_phase_entries_len;
			break;
		}
...
// Add this to your original C file, replacing the existing main function
int main(int argc, char **argv) //parameter output
{
    enum task_name task = FFT; // default
    
    // Parse command line arguments
    if (argc > 1) {
        int task_arg = atoi(argv[1]);
        switch(task_arg) {
            case 0: task = CANNEAL; break;
            case 1: task = DEDUP; break;
            case 2: task = FFT; break;
            case 3: task = STREAMCLUSTER; break;
            case 4: task = RADIOSITY; break;
            default:
                printf("Invalid task number: %d\n", task_arg);
                printf("Valid tasks: 0=CANNEAL, 1=DEDUP, 2=FFT, 3=STREAMCLUSTER, 4=RADIOSITY\n");
                return 1;
        }
    }
    ...
    // Process all cache/membw combinations for the specified task
    for (int c = 0; c < MAX_CACHE_ITR; c++) {
        for (int bw = 0; bw < MAX_MEMBW_ITR; bw++) {
            printf("Processing cache=%d, membw=%d\n", c, bw);
            
            phase_entry_t *entries = get_phase_entries(c, bw, task);
            if (!entries) {
                printf("Warning: Failed to get phase entries for cache=%d, membw=%d\n", c, bw);
                continue;
            }
        }
    }
    
    // Calculate theta entries
    for (int c = 0; c < MAX_CACHE_ITR; c++) {
        for (int bw = 0; bw < MAX_MEMBW_ITR; bw++) {
            printf("Calculating theta for cache=%d, membw=%d\n", c, bw);
            
            phase_entry_t *theta_entries = get_theta_entries(c, bw, task);
            if (!theta_entries) {
                printf("Warning: Failed to get theta entries for cache=%d, membw=%d\n", c, bw);
                continue;
            }
        }
    }
    
    printf("Workload processing completed successfully\n");
...
// Add this function to your original C file
DLL_EXPORT int process_all_tasks(enum task_name task)
{
    printf("Processing all cache/membw combinations for task: %d\n", task);
    
    // Get phase entries for all combinations
    for (int c = 0; c < MAX_CACHE_ITR; c++) {
        for (int bw = 0; bw < MAX_MEMBW_ITR; bw++) {
            phase_entry_t *entries = get_phase_entries(c, bw, task);
            if (!entries) {
                printf("Failed to get phase entries for cache=%d, membw=%d\n", c, bw);
                return -1;
            }
        }
    }
    
    // Calculate theta entries for all combinations
    for (int c = 0; c < MAX_CACHE_ITR; c++) {
        for (int bw = 0; bw < MAX_MEMBW_ITR; bw++) {
            phase_entry_t *theta_entries = get_theta_entries(c, bw, task);
            if (!theta_entries) {
                printf("Failed to get theta entries for cache=%d, membw=%d\n", c, bw);
                return -1;
            }
        }
    }
